...
rq1_pr <- glmer(accepted ~ test_file 
                + total_churn  
                + social_distance 
                + num_comments 
                + prior_interaction 
...
AIC_rq1_pr <- round(AIC(rq1_pr), digits = 2) 
AIC_rq1_pr
odds_ratio_rq1_pr <- round(exp(cbind(odds=fixef(rq1_pr), confint(rq1_pr,parm="beta_",method="Wald", level = 0.99))), 2) # calcola odds ratio
odds_ratio_rq1_pr
index_rq1_pr <- r.squaredGLMM(rq1_pr) # calcola  R2C e R2M
index_rq1_pr
...
print("Boostrapping")
requester_model_boot_pr <- boot(data=data_pr_s, 
                                ...
                                + (1|requester) 
                                + (1|closer))
requester_model_boot_pr

print("Calculating 95% conf. int.")
for (i in 1:20){
  bci <-  boot.ci(requester_model_boot_pr, 
                  conf = 0.95, 
                  index=i, 
...
save(rq1_pr, requester_model_boot_pr, file ="rep-packages/iyer2019tse/RQ1/bootstrap.rda")
