...
rq3_pr <- glmer(accepted ~ test_file 
                + total_churn  
                + social_distance 
                + num_comments 
                + prior_interaction 
                ...
                + stars_current  
                +  diff_openness_abs 
                + diff_conscientiousness_abs 
                + diff_extraversion_abs 
                + diff_agreeableness_abs 
                + diff_neuroticism_abs 
...
AIC_rq3_pr <- round(AIC(rq3_pr), digits = 2)
AIC_rq3_pr
odds_ratio_rq3_pr <- round(exp(cbind(odds=fixef(rq3_pr), confint(rq3_pr,parm="beta_",method="Wald", level = 0.99))), 2) # calcola odds ratio
odds_ratio_rq3_pr
index_rq3_pr <- r.squaredGLMM(rq3_pr)
index_rq3_pr
...
diff_model_boot_pr <- boot(data=data_pr_s, 
...
for (i in 1:20){
  bci <-  boot.ci(diff_model_boot_pr, 
                  conf = 0.95, 
                  index=i, 
...
save(rq3_pr, diff_model_boot_pr, file ="rep-packages/iyer2019tse/RQ3/bootstrap.rda")
